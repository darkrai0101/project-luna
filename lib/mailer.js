...
exports.notiUpdateEmailSuccess = notiUpdateEmailSuccess;
...
function updateEmail(email, token){
  var url = config.constant.url+"/account/auth-email/"+token;
  var html = template.top
  +'<p style="color:#444750">Xin chào,</p>'
  +'<p style="color:#444750">Bạn nhận được email này vì bạn hoặc ai đó đã sử dụng địa chỉ email của bạn để đăng ký nhận thông báo nhắc nhở theo lịch âm tại '+template.homepage+'.</p>'
  +template.button(url,'Xác nhận Email')
  +template.bottom;

  var message = {
      from: 'amduonglich <amduonglich@gmail.com>',
      to: '<"'+email+'">',
      subject: '[Âm Dương Lịch] Xác nhận cập nhật Email', 
      headers: {
      ...
      transport.sendMail(message, function(error){
          ...
          console.log('Message sent successfully auth update email to:'+email);
  });
}


function notiUpdateEmailSuccess(email){

  var html = template.top
      +'<p style="color:#444750">Xin chào,</p>'
      +'<p style="color:#444750">Cám ơn bạn đã sử dụng dịch vụ của chúng tôi. Email của bạn đã được cài đặt, các nhắc nhở được tạo sẽ được gửi về email này.</p>'
      +template.bottom;
  var message = {
      from: 'amduonglich <amduonglich@gmail.com>',
      to: '<"'+email+'">',
      subject: '[Âm Dương Lịch] Cập nhật email thành công', 
      headers: {
          'X-Laziness-level': 1000
      },
      html:html,
    ...
    transport.sendMail(message, function(error){
        ...
        console.log('Message sent successfully noti update email success:'+email);
    });
}
...
function notiMail(isUser, email, desc, time,callback){

  var url = config.constant.url;
  isUser ? url = url+'#/event-list' : url = url+'#/delete';
  ...
  var html = template.top
      ...
      +template.quote((!desc || /^\s*$/.test(desc))?'<span style="font-style:italic">Không có nội dung</span>':desc+'<br/>'+time)
      ...
      +template.button(url,'Xóa nhắc nhở')
      +template.bottom;
  var message = {
      from: 'amduonglich <amduonglich@gmail.com>',
      to: '<"'+email+'">',
