...
function notiMail(email, desc, time){
  var html = template.top
      ...
      +template.quote(desc+'<br/>'+time)
      +'<p style="color:#444750">Nếu bạn không đăng ký nhắc nhở này, xin hãy thông báo cho nhóm chúng tôi thông qua địa chỉ email '+temppate.email+'. Bạn có thể xóa nhắc nhở bằng bút dưới đây.</p>'
...
function deleteEventMail(email, option){
  ...
  for(var p in option){
    var event = option[p];
    html += template.quote(event.message+'<br/>'+event.time)
